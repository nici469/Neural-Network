...
namespace Counter_Console
{
    ...
    /// FAILURE
    /// </summary>
    public class CounterNeuron
    {
        public static Methods M;
        public CounterNeuron( int[] Nodes, int tStep) {
            L = Nodes.Length - 1;
            T = tStep;
            incT = 2;
            lnRate = 0.005 / T;
        ...
        public void ComputeVariables(int l,int t)
        {
            ...
            if (t == 0){ H = new Vector(D);
                Xt_1 = H-1;//set to an initial state of -1
            }
            else { H.data = XJ[l, t - 1];
                Xt_1 = H;
            }
        ...
        public void ComputeVariableGrad(int l, int t)
        {
            ...
            else { xt_1 = (new Vector(D))-1;//at t=1, the initial state of x(t-1) is -1
